...
public class EmployeeController {
  ...
  public ResponseEntity<EmployeeModel> updateStrength(
    ...
    final EmployeeModel employeeModel = service
      ...
      .map(employee -> {
        ...
        final EmployeeInput input = transformerRegistry
          .getPluginFor(Power.STRENGTH, () -> new EmployeeException(MISSING_PLUGINS))
          .toDomain(patchDTO);
  ...
  @Transactional
  public ResponseEntity<EmployeeModel> updateWeakness(
    @PathVariable UUID id,
    @RequestHeader(name = IF_UNMODIFIED_SINCE) Instant ifUnmodifiedSince,
    ...
    final EmployeeModel employeeModel = service
      ...
      .map(employee -> {
        ...
        this.validateDTO(patchDTO);
        final EmployeeInput input = transformerRegistry
          .getPluginFor(Power.WEAKNESS, () -> new EmployeeException(MISSING_PLUGINS))
          .toDomain(patchDTO);
        return service.updateWeakness(creator.toDomain(input), employee);
      })
      .map(assembler::toModel)
      .orElseThrow(() -> new EmployeeException(RESOURCE_NOT_FOUND, id));
    return new ResponseEntity<>(employeeModel, HttpStatus.ACCEPTED);
  ...
  private <T extends EmployeeDTO> void validateDTO(final T employeeDTO) {
    ...
    dataBinder.validate();
    if (dataBinder.getBindingResult().hasErrors()) throw new DatabindingException(dataBinder.getBindingResult());
  }
}
