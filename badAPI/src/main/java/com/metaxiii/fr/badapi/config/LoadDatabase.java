...
public class LoadDatabase {
  ...
  CommandLineRunner initDatabase(EmployeeRepository employeeRepository, SalaryRepository salaryRepository) {
    return args -> {
      final SalaryEntity batmanSalary = salaryRepository.save(
        SalaryEntity.builder().amount(0x7fffffffffffffffL).build()
      );
      final SalaryEntity notBatmanSalary = salaryRepository.save(SalaryEntity.builder().amount(1000000L).build());
